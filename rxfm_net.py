...
class RXFM_Net_Wrapper(nn.Module):
  ...
  def forward( self, x ,ts, tt, att_w):
    input_1, input_2 = x
    output_1 = self.rxfm_net_obj.forward(input_1)
    output_2 = self.rxfm_net_obj.forward(input_2)

    # output_1, temp1= self.att_obj(output_1)   
    # output_2, temp2 = self.att_obj(output_2) 

    means_1, weights_1 = self.sm_obj.forward( output_1 )
    means_2, weights_2 = self.sm_obj.forward( output_2 )


    means_1 = att_w*self.sm_obj.time_encoder(ts) + means_1   #  KM 0.1
    means_2 = att_w*self.sm_obj.time_encoder(tt) + means_2  # EF 0.05
    # # Convert PyTorch tensors to NumPy arrays
    # means_1_np = means_1.reshape(16,12).detach().cpu().numpy()
    # means_2_np = means_2.reshape(16,12).detach().cpu().numpy()

    # # Convert NumPy arrays to PIL Images
    # image_means_1 = Image.fromarray((means_1_np * 255).astype(np.uint8))
    # image_means_2 = Image.fromarray((means_2_np * 255).astype(np.uint8))

    # # Save images with dynamic filenames
    # filename_means_1 = f"means_1_{ts}.png"
    # filename_means_2 = f"means_2_{tt}.png"

    # image_means_1.save(filename_means_1)
    # image_means_2.save(filename_means_2)

    att_1,temp1 = self.att_obj(means_1)
    att_2,temp2 = self.att_obj(means_2)
    att_total = att_1 + att_2 # For feature fusion, only addition works here
    means_1 = att_w*att_total + means_1
    means_2 = att_w*att_total + means_2
    # print (means_1.shape)
